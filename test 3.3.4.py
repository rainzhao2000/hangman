...
class MainApp(tk.Tk):
    ...
    def menu(self):
        ...
        self.canvas.pack()
        self.main_photo = tk.PhotoImage(file='main.gif')
        self.main_photo = self.main_photo
        self.canvas.create_image(0,0, image=self.main_photo, anchor=tk.NW)
...
the word""", font=('Calibri', 12), justify=tk.LEFT)
    ...
    def win(self):
        win_gui = AppEnd(self, "You Win!", '450x550', 550, 'win.gif', 430, 500,
                         'win')

    def lose(self):
        lose_gui = AppEnd(self, "You Lose :(", '450x500', 500, 'lose.gif', 380,
                          450, 'lose')

class AppEnd(tk.Toplevel, MainApp):
    def __init__(self, master, end_title, end_geometry, canvas_height, end_pic,
        ...
        self.canvas = tk.Canvas(self, width=450, height=self.canvas_height)
        self.canvas.pack()
        self.end_pic = end_pic
        self.end_photo = tk.PhotoImage(file=self.end_pic)
        self.canvas.create_image(0,0, image=self.end_photo, anchor=tk.NW)
